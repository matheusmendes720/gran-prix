# 📊 Deep Technical Documentation: Anatel Broadband Accesses
...
**Name:** Anatel Broadband Accesses
**Description:** Municipality-level broadband connection data from Anatel. Coverage, speeds, and technology breakdown.

**URL:** [https://data-basis.org/search?organization=anatel](https://data-basis.org/search?organization=anatel)
...
Anatel official broadband data. Municipality-level granularity for spatial demand forecasting.
...
```bash
# Download dataset
python backend/scripts/fetch_all_ml_datasets.py --dataset anatel_broadband

# Structure for ML
python backend/scripts/structure_ml_datasets.py --dataset anatel_broadband

# Enrich with external factors
python backend/scripts/comprehensive_dataset_pipeline.py --dataset anatel_broadband
```
