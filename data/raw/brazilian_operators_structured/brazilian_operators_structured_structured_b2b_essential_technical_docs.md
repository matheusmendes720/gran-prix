...
## Brazilian Telecom Operators Market Data
...
**Purpose:** Monthly operator market data from 2019-2024: Vivo, Claro, TIM, Others. Subscribers, market share, 5G coverage.  
**Records:** 288 rows  
**Features:** 8 columns  
...
Monthly operator market data from 2019-2024: Vivo, Claro, TIM, Others. Subscribers, market share, 5G coverage.
...
| `operator` | String | - | operator column | Mapped to unified schema |
| `subscribers_millions` | String | - | subscribers_millions column | Mapped to unified schema |
| `market_share` | String | - | market_share column | Data field |
| `revenue_growth_rate` | String | - | revenue_growth_rate column | Data field |
| `5g_coverage_pct` | String | - | 5g_coverage_pct column | Data field |
...
```python
# Brazilian Operators Structured → Nova Corrente Demand Forecasting

def brazilian_operators_structured_to_demand(data):
    """
    Maps brazilian_operators_structured data to Nova Corrente demand forecasting format
...
**Use Case:** Brazilian Operators Structured → Nova Corrente demand forecasting for telecom equipment logistics  
...
- `data/raw/brazilian_operators_structured/*.csv` (CSV format)

**Processed Data:**

- `data/processed/ml_ready/brazilian_operators_structured_structured.csv`
...
**Original Notes:** Generated structured data from public reports. Market share dynamics: Vivo (32%), Claro (27%), TIM (20%). Essential for competitive analysis.
...
**Key Insight:** Brazilian Operators Structured provides valuable insights for Nova Corrente demand forecasting in Brazilian telecom logistics market
...
**Basic Statistics:**
- **Sample Size:** 288 records (Small dataset)
- **Feature Count:** 8 features (Low-dimensional)
- **Temporal Coverage:** 2019-01-31 00:00:00 to 2024-12-31 00:00:00
...
# Using: brazilian_operators_structured
...
**Brazilian Operators Structured + Other Datasets:**
...
```python
# Example: Integrate brazilian_operators_structured with demand forecasting
import pandas as pd

# Load datasets
demand_df = pd.read_csv('data/processed/ml_ready/demand_structured.csv')
brazilian_operators_structured_df = pd.read_csv('data/processed/ml_ready/brazilian_operators_structured_structured.csv')

# Merge on date
merged_df = demand_df.merge(
    brazilian_operators_structured_df,
    on='date',
    how='left',
...
**Sample Size:**
- ⚠️ Small sample size (288 records): Limited statistical power
