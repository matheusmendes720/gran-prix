...
**Purpose:** 73,000+ daily inventory/demand records with categories  
**Records:** 1000 rows  
**Features:** 15 columns  
**Date Range:** 2022-01-01 00:00:00 to 2022-01-10 00:00:00  
**Target Variable:** `Demand Forecast` - Target variable for demand forecasting

**Business Context - Deep Analysis:**

73,000+ daily inventory/demand records with categories
...
**Platform:** Kaggle  
**Dataset ID:** anirudhchauhan/retail-store-inventory-forecasting-dataset  
...
|--------|------|-------|-------------|------------------|
| `Date` | String | - | Date column | Mapped to unified schema |
| `Store ID` | String | - | Store ID column | Data field |
| `Product ID` | String | - | Product ID column | Data field |
| `Category` | String | - | Category column | Mapped to unified schema |
| `Region` | String | - | Region column | Data field |
| `Inventory Level` | String | - | Inventory Level column | Data field |
| `Units Sold` | String | - | Units Sold column | Data field |
| `Units Ordered` | String | - | Units Ordered column | Data field |
| `Demand Forecast` | String | - | Demand Forecast column | Data field |
| `Price` | String | - | Price column | Data field |
| `Discount` | String | - | Discount column | Data field |
| `Weather Condition` | String | - | Weather Condition column | Data field |
| `Holiday/Promotion` | String | - | Holiday/Promotion column | Data field |
| `Competitor Pricing` | String | - | Competitor Pricing column | Data field |
| `Seasonality` | String | - | Seasonality column | Data field |
...
```python
# Kaggle Retail Inventory → Nova Corrente Demand Forecasting

def kaggle_retail_inventory_to_demand(data):
    """
    Maps kaggle_retail_inventory data to Nova Corrente demand forecasting format
    ...
    base_demand = data.get('Demand Forecast', 0)
...
**Use Case:** Kaggle Retail Inventory → Nova Corrente demand forecasting for telecom equipment logistics  
...
- `data/raw/kaggle_retail_inventory/*.csv` (CSV format)

**Processed Data:**

- `data/processed/ml_ready/kaggle_retail_inventory_structured.csv`
...
**Original Notes:** Large scale dataset, test complex models (LSTM)
...
**Key Insight:** Kaggle Retail Inventory provides valuable insights for Nova Corrente demand forecasting in Brazilian telecom logistics market
...
- **Feature Count:** 15 features (Low-dimensional)
- **Temporal Coverage:** 2022-01-01 00:00:00 to 2022-01-10 00:00:00
- **Target Variable:** `Demand Forecast`
...
**Kaggle Retail Inventory + Other Datasets:**
...
```python
# Example: Integrate kaggle_retail_inventory with demand forecasting
import pandas as pd

# Load datasets
demand_df = pd.read_csv('data/processed/ml_ready/demand_structured.csv')
kaggle_retail_inventory_df = pd.read_csv('data/processed/ml_ready/kaggle_retail_inventory_structured.csv')

# Merge on date
merged_df = demand_df.merge(
    kaggle_retail_inventory_df,
    on='date',
    how='left',
