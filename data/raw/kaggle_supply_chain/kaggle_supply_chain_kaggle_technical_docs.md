...
## High-Dimensional Supply Chain Inventory
...
**Purpose:** Hundreds of thousands of supply chain records  
**Records:** 1000 rows  
**Features:** 15 columns  
**Date Range:** 2024-01-01 00:00:00 to 2024-12-30 00:00:00  
**Target Variable:** `Order_Quantity` - Target variable for demand forecasting

**Business Context - Deep Analysis:**

Hundreds of thousands of supply chain records
...
**Platform:** Kaggle  
**Dataset ID:** ziya07/high-dimensional-supply-chain-inventory-dataset  
...
| `SKU_ID` | String | - | SKU_ID column | Data field |
| `Warehouse_ID` | String | - | Warehouse_ID column | Data field |
| `Supplier_ID` | String | - | Supplier_ID column | Data field |
| `Region` | String | - | Region column | Data field |
| `Units_Sold` | String | - | Units_Sold column | Data field |
| `Inventory_Level` | String | - | Inventory_Level column | Data field |
| `Supplier_Lead_Time_Days` | String | - | Supplier_Lead_Time_Days column | Data field |
| `Reorder_Point` | String | - | Reorder_Point column | Data field |
| `Order_Quantity` | String | - | Order_Quantity column | Data field |
| `Unit_Cost` | String | - | Unit_Cost column | Data field |
| `Unit_Price` | String | - | Unit_Price column | Data field |
| `Promotion_Flag` | String | - | Promotion_Flag column | Data field |
| `Stockout_Flag` | String | - | Stockout_Flag column | Data field |
...
```python
# Kaggle Supply Chain → Nova Corrente Demand Forecasting

def kaggle_supply_chain_to_demand(data):
    """
    Maps kaggle_supply_chain data to Nova Corrente demand forecasting format
    ...
    base_demand = data.get('Order_Quantity', 0)
...
**Use Case:** Kaggle Supply Chain → Nova Corrente demand forecasting for telecom equipment logistics  
...
- `data/raw/kaggle_supply_chain/*.csv` (CSV format)

**Processed Data:**

- `data/processed/ml_ready/kaggle_supply_chain_structured.csv`
...
**Original Notes:** Includes external factors, multi-location operations
...
**Key Insight:** Kaggle Supply Chain provides valuable insights for Nova Corrente demand forecasting in Brazilian telecom logistics market
...
- **Temporal Coverage:** 2024-01-01 00:00:00 to 2024-12-30 00:00:00
- **Target Variable:** `Order_Quantity`
...
**Kaggle Supply Chain + Other Datasets:**
...
```python
# Example: Integrate kaggle_supply_chain with demand forecasting
import pandas as pd

# Load datasets
demand_df = pd.read_csv('data/processed/ml_ready/demand_structured.csv')
kaggle_supply_chain_df = pd.read_csv('data/processed/ml_ready/kaggle_supply_chain_structured.csv')

# Merge on date
merged_df = demand_df.merge(
    kaggle_supply_chain_df,
    on='date',
    how='left',
