...
## Brazilian IoT Market Structured Data
...
**Purpose:** Structured IoT connection data from 2020-2024, monthly granularity, sector breakdown (agriculture, logistics, smart cities, utilities, retail).  
**Records:** 300 rows  
**Features:** 7 columns  
...
Structured IoT connection data from 2020-2024, monthly granularity, sector breakdown (agriculture, logistics, smart cities, utilities, retail).
...
| `sector` | String | - | sector column | Mapped to unified schema |
| `iot_connections_millions` | String | - | iot_connections_millions column | Mapped to unified schema |
| `growth_rate_annual` | String | - | growth_rate_annual column | Data field |
...
```python
# Brazilian Iot Structured → Nova Corrente Demand Forecasting

def brazilian_iot_structured_to_demand(data):
    """
    Maps brazilian_iot_structured data to Nova Corrente demand forecasting format
...
**Use Case:** Brazilian Iot Structured → Nova Corrente demand forecasting for telecom equipment logistics  
...
- `data/raw/brazilian_iot_structured/*.csv` (CSV format)

**Processed Data:**

- `data/processed/ml_ready/brazilian_iot_structured_structured.csv`
...
1. **Schema Normalization:**
   - Column mapping to unified schema: {
    "date": "date",
    "item_id": "sector",
    "quantity": "iot_connections_millions",
    "category": "sector",
    "site_id": "region"
}
...
**Original Notes:** Generated structured data based on industry research. 28M to 46.2M connections growth. Perfect for IoT demand forecasting.
...
**Key Insight:** Brazilian Iot Structured provides valuable insights for Nova Corrente demand forecasting in Brazilian telecom logistics market
...
**Basic Statistics:**
- **Sample Size:** 300 records (Small dataset)
- **Feature Count:** 7 features (Low-dimensional)
- **Temporal Coverage:** 2020-01-31 00:00:00 to 2024-12-31 00:00:00
...
# Using: brazilian_iot_structured
...
**Brazilian Iot Structured + Other Datasets:**
...
```python
# Example: Integrate brazilian_iot_structured with demand forecasting
import pandas as pd

# Load datasets
demand_df = pd.read_csv('data/processed/ml_ready/demand_structured.csv')
brazilian_iot_structured_df = pd.read_csv('data/processed/ml_ready/brazilian_iot_structured_structured.csv')

# Merge on date
merged_df = demand_df.merge(
    brazilian_iot_structured_df,
    on='date',
    how='left',
...
**Sample Size:**
- ⚠️ Small sample size (300 records): Limited statistical power
