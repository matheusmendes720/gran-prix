...
**Purpose:** Sample dataset for testing pipeline (730 days, 2 years)  
**Records:** 730 rows  
**Features:** 7 columns  
**Date Range:** 2023-01-01 00:00:00 to 2024-12-30 00:00:00  
**Target Variable:** `Order_Demand` - Target variable for demand forecasting

**Business Context - Deep Analysis:**

Sample dataset for testing pipeline (730 days, 2 years)
...
- Available through Test platform
...
| `Product` | String | - | Product column | Mapped to unified schema |
| `Order_Demand` | String | - | Order_Demand column | Mapped to unified schema |
| `Site` | String | - | Site column | Mapped to unified schema |
| `Category` | String | - | Category column | Mapped to unified schema |
| `Cost` | String | - | Cost column | Mapped to unified schema |
| `Lead_Time` | String | - | Lead_Time column | Mapped to unified schema |
...
```python
# Test Dataset → Nova Corrente Demand Forecasting

def test_dataset_to_demand(data):
    """
    Maps test_dataset data to Nova Corrente demand forecasting format
    ...
    base_demand = data.get('Order_Demand', 0)
...
**Context:** Original Test dataset analysis for telecom logistics  
...
**Use Case:** Test Dataset → Nova Corrente demand forecasting for telecom equipment logistics  
...
- `data/raw/test_dataset/*.csv` (CSV format)

**Processed Data:**

- `data/processed/ml_ready/test_dataset_structured.csv`
...
**Original Notes:** Test dataset with 2 years of daily data
...
**Key Insight:** Test Dataset provides valuable insights for Nova Corrente demand forecasting in Brazilian telecom logistics market
...
**Basic Statistics:**
- **Sample Size:** 730 records (Small dataset)
- **Feature Count:** 7 features (Low-dimensional)
- **Temporal Coverage:** 2023-01-01 00:00:00 to 2024-12-30 00:00:00
...
**Test Dataset + Other Datasets:**

**Recommended Integrations:**
1. **With Demand Datasets:**
   - Enrich demand data with test features
...
```python
# Example: Integrate test_dataset with demand forecasting
import pandas as pd

# Load datasets
demand_df = pd.read_csv('data/processed/ml_ready/demand_structured.csv')
test_dataset_df = pd.read_csv('data/processed/ml_ready/test_dataset_structured.csv')
...
merged_df = demand_df.merge(
    ...
    on='date',
    how='left',
    suffixes=('_demand', '_test')
)

# Feature engineering with integrated data
merged_df['test_adjusted_demand'] = (
    merged_df['quantity'] * 
    merged_df['test_multiplier'].fillna(1.0)
)
...
**Sample Size:**
- ⚠️ Small sample size (730 records): Limited statistical power
