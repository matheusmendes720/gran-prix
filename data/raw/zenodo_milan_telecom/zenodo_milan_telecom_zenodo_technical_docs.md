...
**Purpose:** Telecom traffic time-series with weather data (precipitation, temperature). Contains 5G network slice resource demand prediction data.  
**Records:** 1000 rows  
**Features:** 38 columns  
**Date Range:** Unknown  
...
Telecom traffic time-series with weather data (precipitation, temperature). Contains 5G network slice resource demand prediction data.

### Dataset Origin & Historical Context

**Origin:** Academic research repository (Zenodo/CERN)
...
**Research Team:** See Zenodo record for full research details  
**Paper:** Related paper title available in Zenodo metadata  
**DOI:** 10.5281/zenodo.14012612
...
|--------|------|-------|-------------|------------------|
| `bsId` | String | - | bsId column | Mapped to unified schema |
| `episode` | String | - | episode column | Data field |
| `step` | String | - | step column | Mapped to unified schema |
| `loadSMS` | String | - | loadSMS column | Data field |
| `rawActSMS` | String | - | rawActSMS column | Data field |
| `loadInt` | String | - | loadInt column | Data field |
| `rawActInt` | String | - | rawActInt column | Data field |
| `loadCalls` | String | - | loadCalls column | Data field |
| `rawActCalls` | String | - | rawActCalls column | Data field |
| `defSMS` | String | - | defSMS column | Data field |
| `rawActDefSMS` | String | - | rawActDefSMS column | Data field |
| `defInt` | String | - | defInt column | Data field |
| `rawActDefInt` | String | - | rawActDefInt column | Data field |
| `defCalls` | String | - | defCalls column | Data field |
| `rawActDefCalls` | String | - | rawActDefCalls column | Data field |
| `schedSMS` | String | - | schedSMS column | Data field |
| `schedInt` | String | - | schedInt column | Data field |
| `schedCalls` | String | - | schedCalls column | Data field |
| `schedDelaySMS` | String | - | schedDelaySMS column | Data field |
| `schedDelayInt` | String | - | schedDelayInt column | Data field |
| ... | ... | ... | (18 more columns) | ... |
...
```python
# Zenodo Milan Telecom → Nova Corrente Demand Forecasting

def zenodo_milan_telecom_to_demand(data):
    """
    Maps zenodo_milan_telecom data to Nova Corrente demand forecasting format
...
**Context:** Original Zenodo dataset analysis for telecom logistics  
...
**Use Case:** Zenodo Milan Telecom → Nova Corrente demand forecasting for telecom equipment logistics  
...
- `data/raw/zenodo_milan_telecom/*.csv` (CSV format)

**Processed Data:**

- `data/processed/ml_ready/zenodo_milan_telecom_structured.csv`
...
**Original Notes:** Includes 5G base station traffic load data (SMS, Internet, Calls). Step column can be used as time index. Includes external factors like capacity, reject rates, etc. Perfect for SARIMAX.
...
1. **Research papers** - See Zenodo repository for full research documentation  
2. **Related academic papers** - Check DOI references in Zenodo metadata

### Similar Datasets

- Zenodo - Similar academic datasets
...
**Key Insight:** Zenodo Milan Telecom provides valuable insights for Nova Corrente demand forecasting in Brazilian telecom logistics market
...
- **Feature Count:** 38 features (Standard)
...
**Zenodo Milan Telecom + Other Datasets:**

**Recommended Integrations:**
1. **With Demand Datasets:**
   - Enrich demand data with zenodo features
...
```python
# Example: Integrate zenodo_milan_telecom with demand forecasting
import pandas as pd

# Load datasets
demand_df = pd.read_csv('data/processed/ml_ready/demand_structured.csv')
zenodo_milan_telecom_df = pd.read_csv('data/processed/ml_ready/zenodo_milan_telecom_structured.csv')

# Merge on date
merged_df = demand_df.merge(
    zenodo_milan_telecom_df,
    on='date',
    how='left',
    suffixes=('_demand', '_zenodo')
)

# Feature engineering with integrated data
merged_df['zenodo_adjusted_demand'] = (
    merged_df['quantity'] * 
    merged_df['zenodo_multiplier'].fillna(1.0)
)
