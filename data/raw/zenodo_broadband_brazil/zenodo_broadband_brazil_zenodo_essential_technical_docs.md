...
**Purpose:** Extracted from a major Brazilian operator, includes modem parameters (latency, jitter, packet loss) and user metrics for thousands of broadband users. Time-series snapshots.  
**Records:** 1000 rows  
**Features:** 8 columns  
**Date Range:** Unknown  
...
Extracted from a major Brazilian operator, includes modem parameters (latency, jitter, packet loss) and user metrics for thousands of broadband users. Time-series snapshots.
...
|--------|------|-------|-------------|------------------|
| `Customer_ID` | String | - | Customer_ID column | Mapped to unified schema |
| `Latency` | String | - | Latency column | Data field |
| `Jitter` | String | - | Jitter column | Data field |
| `Packet_Loss` | String | - | Packet_Loss column | Mapped to unified schema |
| `Channel2_quality` | String | - | Channel2_quality column | Mapped to unified schema |
| `Channel5_quality` | String | - | Channel5_quality column | Data field |
| `N_distant_devices` | String | - | N_distant_devices column | Data field |
| `CRM_Complaint?` | String | - | CRM_Complaint? column | Data field |
...
```python
# Zenodo Broadband Brazil → Nova Corrente Demand Forecasting

def zenodo_broadband_brazil_to_demand(data):
    """
    Maps zenodo_broadband_brazil data to Nova Corrente demand forecasting format
...
**Use Case:** Zenodo Broadband Brazil → Nova Corrente demand forecasting for telecom equipment logistics  
...
- `data/raw/zenodo_broadband_brazil/*.csv` (CSV format)

**Processed Data:**

- `data/processed/ml_ready/zenodo_broadband_brazil_structured.csv`
...
**Original Notes:** Real Brazilian operator data with customer metrics (latency, jitter, packet loss, channel quality). Predictive modeling for demand in network maintenance. Long-tail insights into rare downtime events. No date column - may need temporal indexing. Operator-specific bias may exist.
...
**Key Insight:** Zenodo Broadband Brazil provides valuable insights for Nova Corrente demand forecasting in Brazilian telecom logistics market
...
- **Feature Count:** 8 features (Low-dimensional)
...
**Zenodo Broadband Brazil + Other Datasets:**
...
```python
# Example: Integrate zenodo_broadband_brazil with demand forecasting
import pandas as pd

# Load datasets
demand_df = pd.read_csv('data/processed/ml_ready/demand_structured.csv')
zenodo_broadband_brazil_df = pd.read_csv('data/processed/ml_ready/zenodo_broadband_brazil_structured.csv')

# Merge on date
merged_df = demand_df.merge(
    zenodo_broadband_brazil_df,
    on='date',
    how='left',
