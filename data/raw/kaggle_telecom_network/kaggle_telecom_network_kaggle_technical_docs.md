...
**Purpose:** Tower-level operations data with tower ID, connected users, download speeds, and performance indicators. Supports logistics for capacity planning and demand forecasting.  
**Records:** 1000 rows  
**Features:** 8 columns  
...
Tower-level operations data with tower ID, connected users, download speeds, and performance indicators. Supports logistics for capacity planning and demand forecasting.
...
| `tower_id` | String | - | tower_id column | Mapped to unified schema |
| `users_connected` | String | - | users_connected column | Mapped to unified schema |
| `download_speed` | String | - | download_speed column | Data field |
| `upload_speed` | String | - | upload_speed column | Data field |
| `latency` | String | - | latency column | Data field |
| `weather` | String | - | weather column | Data field |
| `congestion` | String | - | congestion column | Data field |
...
```python
# Kaggle Telecom Network → Nova Corrente Demand Forecasting

def kaggle_telecom_network_to_demand(data):
    """
    Maps kaggle_telecom_network data to Nova Corrente demand forecasting format
...
**Use Case:** Kaggle Telecom Network → Nova Corrente demand forecasting for telecom equipment logistics  
...
- `data/raw/kaggle_telecom_network/*.csv` (CSV format)

**Processed Data:**

- `data/processed/ml_ready/kaggle_telecom_network_structured.csv`
...
**Original Notes:** Capacity utilization data for logistics planning. Long-tail aspects emerge in low-traffic tower maintenance patterns.
...
**Key Insight:** Kaggle Telecom Network provides valuable insights for Nova Corrente demand forecasting in Brazilian telecom logistics market
...
- **Temporal Coverage:** 2025-01-01 00:00:00 to 2025-01-31 00:00:00
...
**Kaggle Telecom Network + Other Datasets:**
...
```python
# Example: Integrate kaggle_telecom_network with demand forecasting
import pandas as pd

# Load datasets
demand_df = pd.read_csv('data/processed/ml_ready/demand_structured.csv')
kaggle_telecom_network_df = pd.read_csv('data/processed/ml_ready/kaggle_telecom_network_structured.csv')

# Merge on date
merged_df = demand_df.merge(
    kaggle_telecom_network_df,
    on='date',
    how='left',
