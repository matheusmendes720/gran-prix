...
## Cloud-Based Supply Chain Demand Dataset
...
**Purpose:** Real-time data from cloud-based environment for demand forecasting and inventory management. Features: 3,204 records of historical sales, inventory levels, supplier details, lead times, and costs.  
**Records:** 1000 rows  
**Features:** 14 columns  
**Date Range:** 2024-01-01 00:00:00 to 2024-06-28 00:00:00  
**Target Variable:** `restock_quantity` - Target variable for demand forecasting

**Business Context - Deep Analysis:**

Real-time data from cloud-based environment for demand forecasting and inventory management. Features: 3,204 records of historical sales, inventory levels, supplier details, lead times, and costs.
...
**Platform:** Kaggle  
**Dataset ID:** programmer3/cloud-based-supply-chain-demand-dataset  
**URL:** https://www.kaggle.com/datasets/programmer3/cloud-based-supply-chain-demand-dataset  
...
| `product_id` | String | - | product_id column | Mapped to unified schema |
| `product_category` | String | - | product_category column | Data field |
| `warehouse_location` | String | - | warehouse_location column | Data field |
| `inventory_level` | String | - | inventory_level column | Data field |
| `units_sold` | String | - | units_sold column | Mapped to unified schema |
...
| `restock_quantity` | String | - | restock_quantity column | Data field |
| `promotion_flag` | String | - | promotion_flag column | Data field |
| `temperature_celsius` | String | - | temperature_celsius column | Data field |
| `humidity_percent` | String | - | humidity_percent column | Data field |
| `truck_arrival_delay_mins` | String | - | truck_arrival_delay_mins column | Data field |
...
```python
# Kaggle Cloud Supply Chain → Nova Corrente Demand Forecasting

def kaggle_cloud_supply_chain_to_demand(data):
    """
    Maps kaggle_cloud_supply_chain data to Nova Corrente demand forecasting format
    ...
    base_demand = data.get('restock_quantity', 0)
...
**Use Case:** Kaggle Cloud Supply Chain → Nova Corrente demand forecasting for telecom equipment logistics  
...
- `data/raw/kaggle_cloud_supply_chain/*.csv` (CSV format)

**Processed Data:**

- `data/processed/ml_ready/kaggle_cloud_supply_chain_structured.csv`
...
**Original Notes:** Directly supports telecom logistics by modeling demand for hardware shipments; adaptable for long-tail items via variability analysis.
...
**Key Insight:** Kaggle Cloud Supply Chain provides valuable insights for Nova Corrente demand forecasting in Brazilian telecom logistics market
...
- **Feature Count:** 14 features (Low-dimensional)
- **Temporal Coverage:** 2024-01-01 00:00:00 to 2024-06-28 00:00:00
- **Target Variable:** `restock_quantity`
...
# Using: kaggle_cloud_supply_chain
...
**Kaggle Cloud Supply Chain + Other Datasets:**
...
```python
# Example: Integrate kaggle_cloud_supply_chain with demand forecasting
import pandas as pd

# Load datasets
demand_df = pd.read_csv('data/processed/ml_ready/demand_structured.csv')
kaggle_cloud_supply_chain_df = pd.read_csv('data/processed/ml_ready/kaggle_cloud_supply_chain_structured.csv')

# Merge on date
merged_df = demand_df.merge(
    kaggle_cloud_supply_chain_df,
    on='date',
    how='left',
