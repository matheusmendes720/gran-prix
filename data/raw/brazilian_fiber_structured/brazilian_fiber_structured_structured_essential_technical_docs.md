...
## Brazilian Fiber Optic Expansion Structured Data
...
**Purpose:** Quarterly fiber household penetration data from 2020-2024 by region (Southeast, South, Northeast, North, Central West).  
**Records:** 100 rows  
**Features:** 7 columns  
**Date Range:** 2020-03-31 00:00:00 to 2024-12-31 00:00:00  
**Target Variable:** `quantity` - Target variable for demand forecasting

**Business Context - Deep Analysis:**

Quarterly fiber household penetration data from 2020-2024 by region (Southeast, South, Northeast, North, Central West).
...
| `quarter` | String | - | quarter column | Data field |
| `region` | String | - | region column | Mapped to unified schema |
| `household_penetration` | String | - | household_penetration column | Data field |
| `estimated_households_millions` | String | - | estimated_households_millions column | Mapped to unified schema |
| `growth_rate` | String | - | growth_rate column | Data field |
...
- ⚠️ Small dataset size (may need more data)
- ⚠️ Target variable needs identification/mapping

**Adaptation Strategy:**

```python
# Brazilian Fiber Structured → Nova Corrente Demand Forecasting

def brazilian_fiber_structured_to_demand(data):
    """
    Maps brazilian_fiber_structured data to Nova Corrente demand forecasting format
...
**Use Case:** Brazilian Fiber Structured → Nova Corrente demand forecasting for telecom equipment logistics  
...
- `data/raw/brazilian_fiber_structured/*.csv` (CSV format)

**Processed Data:**

- `data/processed/ml_ready/brazilian_fiber_structured_structured.csv`
...
1. **Schema Normalization:**
   - Column mapping to unified schema: {
    "date": "date",
    "item_id": "region",
    "quantity": "estimated_households_millions",
    "category": "region",
    "site_id": "region"
}
...
**Original Notes:** Generated structured data from market reports. Household penetration 25% (2020) to 49% (2024). Regional breakdown essential for infrastructure planning.
...
**Key Insight:** Brazilian Fiber Structured provides valuable insights for Nova Corrente demand forecasting in Brazilian telecom logistics market
...
**Basic Statistics:**
- **Sample Size:** 100 records (Small dataset)
- **Feature Count:** 7 features (Low-dimensional)
- **Temporal Coverage:** 2020-03-31 00:00:00 to 2024-12-31 00:00:00
...
# Using: brazilian_fiber_structured
...
**Brazilian Fiber Structured + Other Datasets:**
...
```python
# Example: Integrate brazilian_fiber_structured with demand forecasting
import pandas as pd

# Load datasets
demand_df = pd.read_csv('data/processed/ml_ready/demand_structured.csv')
brazilian_fiber_structured_df = pd.read_csv('data/processed/ml_ready/brazilian_fiber_structured_structured.csv')

# Merge on date
merged_df = demand_df.merge(
    brazilian_fiber_structured_df,
    on='date',
    how='left',
...
**Sample Size:**
- ⚠️ Small sample size (100 records): Limited statistical power
