...
## Equipment Failure Prediction Dataset
...
**Purpose:** 10,000 points with 14 features on machine failures (hardware/software types). Models long-tail failures in telecom equipment, informing demand for spare parts.  
**Records:** 1000 rows  
**Features:** 14 columns  
**Date Range:** Unknown  
...
10,000 points with 14 features on machine failures (hardware/software types). Models long-tail failures in telecom equipment, informing demand for spare parts.
...
|--------|------|-------|-------------|------------------|
| `UDI` | String | - | UDI column | Data field |
| `Product ID` | String | - | Product ID column | Mapped to unified schema |
| `Type` | String | - | Type column | Mapped to unified schema |
| `Air temperature [K]` | String | - | Air temperature [K] column | Data field |
| `Process temperature [K]` | String | - | Process temperature [K] column | Data field |
| `Rotational speed [rpm]` | String | - | Rotational speed [rpm] column | Data field |
| `Torque [Nm]` | String | - | Torque [Nm] column | Data field |
| `Tool wear [min]` | String | - | Tool wear [min] column | Data field |
| `Machine failure` | String | - | Machine failure column | Mapped to unified schema |
| `TWF` | String | - | TWF column | Data field |
| `HDF` | String | - | HDF column | Data field |
| `PWF` | String | - | PWF column | Data field |
| `OSF` | String | - | OSF column | Data field |
| `RNF` | String | - | RNF column | Data field |
...
```python
# Kaggle Equipment Failure → Nova Corrente Demand Forecasting

def kaggle_equipment_failure_to_demand(data):
    """
    Maps kaggle_equipment_failure data to Nova Corrente demand forecasting format
...
**Use Case:** Kaggle Equipment Failure → Nova Corrente demand forecasting for telecom equipment logistics  
...
- `data/raw/kaggle_equipment_failure/*.csv` (CSV format)

**Processed Data:**

- `data/processed/ml_ready/kaggle_equipment_failure_structured.csv`
...
**Original Notes:** Models long-tail equipment failures driving logistics demands for spare parts. Classification features for failure types. Useful for rare event prediction.
...
**Key Insight:** Kaggle Equipment Failure provides valuable insights for Nova Corrente demand forecasting in Brazilian telecom logistics market
...
# Using: kaggle_equipment_failure
...
**Kaggle Equipment Failure + Other Datasets:**
...
```python
# Example: Integrate kaggle_equipment_failure with demand forecasting
import pandas as pd

# Load datasets
demand_df = pd.read_csv('data/processed/ml_ready/demand_structured.csv')
kaggle_equipment_failure_df = pd.read_csv('data/processed/ml_ready/kaggle_equipment_failure_structured.csv')

# Merge on date
merged_df = demand_df.merge(
    kaggle_equipment_failure_df,
    on='date',
    how='left',
