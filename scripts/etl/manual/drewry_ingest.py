"""
Normalize manually downloaded Drewry World Container Index CSV files.
...
Usage:
    python -m scripts.etl.manual.drewry_ingest --source data/manual/drewry/wci_global.csv
"""
...
logger = configure_logging("drewry_ingest")

LANDING_ROOT = Path("data/landing/external_factors-raw/logistics/drewry")
START_DATE = pd.Timestamp("2021-01-01")

COLUMN_ALIASES = {
    "date": {"date", "Date"},
    "value": {"value", "Value", "price", "Price", "index", "Index", "WCI"},
}


def parse_args(args: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ingest manual Drewry WCI CSV.")
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not date_col or not value_col:
        raise ValueError(
            "Source file must contain a date column and value/index column."
        )
...
def ingest(source_path: Path, run_stamp: str) -> Path:
    ...
    target_path = LANDING_ROOT / run_stamp / "drewry_wci.csv"
