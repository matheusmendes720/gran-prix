...
try:
    ...
    backfill_records = []
    items = nova_corrente['item_id'].unique()[:10]  # Limit to first 10 items for testing
    sites = nova_corrente['site_id'].unique()[:5]   # Limit to first 5 sites for testing
    
    # Get patterns from current data
    item_stats = nova_corrente.groupby('item_id')['quantidade'].agg(['mean', 'std']).fillna(0).to_dict()
    ...
    # Filter original to same items/sites for consistency
    nova_filtered = nova_corrente[
        (nova_corrente['item_id'].isin(items)) & 
        (nova_corrente['site_id'].isin(sites))
    ].copy()
    
    combined = pd.concat([historical_data, nova_filtered], ignore_index=True)
    ...
    print(f"Extended timeline from 381 to {total_days} days")
